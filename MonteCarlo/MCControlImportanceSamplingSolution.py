...
def generate_episode_from_state(env, s0, a0, policy):
    '''
    Generates an episode using policy with exploring starts (s0,a0)
    Args:
        env     : openAI gym environment
        S0      : Initial state
        a0      : First action
    ...
    episode = []
    state = env.reset()
    next_state, reward, done, _ = env.step(a0)
...
def make_random_policy(nA):
    """
    Creates an random policy.
    Args:
        nA:     Number of actions in the environment.
    Returns:
        A function that takes the observation as an argument and returns
        the probabilities for each action based on a uniform distribution.    
    """
...
def mc_control_importance_sampling(env, num_episodes, discount_factor=1.0):
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for state, action, reward in reversed(episode):
            ...
            # Ref: Sutton & Barto chapter 5.7 p. 111
...
if __name__ == '__main__':
    ...
    """
    Q, policy = mc_control_importance_sampling(env, num_episodes=1000000, discount_factor=1.0)
    V,P = QtoVP(Q)
    plotting.plot_value_function(V, title="MC Importance Sampling - Optimal Value Function - 1m")
    plotting.plot_value_function(P, title="MC Importance Sampling - Optimal Policy - 1m")
